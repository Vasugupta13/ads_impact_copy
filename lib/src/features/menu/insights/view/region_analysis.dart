import 'package:ads/src/class/region_analysis.dart';
...
class _RegionAnalysisState extends State<RegionAnalysis> {
  ...
  void updateTable() {
    filteredData = RegionAnalysisDetail.placedetails
        .map((category) => {
              "place": category["place"].toString(),
  ...
  Widget build(BuildContext context) {
    ...
    double total = RegionAnalysisDetail.placedetails.fold(
        0.0,
        (sum, category) =>
            sum +
            double.parse(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(
            children: [
              const CustomAppBar(
                  name: "Region Analysis", imagepath: IconAssets.analysis),
              height5,
